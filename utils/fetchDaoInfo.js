...
export async function fetchStaticInfo(instance, factory, address, web3, daoChain, account) {
  let dao_
  try {
...
export async function fetchExtensions(instance, daoChain, web3, address, balances) {
  let result
...
async function fetchCrowdsale(web3, address, extAddress, balances) {
  ...
  for (var i = 0; i < balances.length; i++) {
    if (
      web3.utils.toChecksumAddress(balances[i]['address']) == web3.utils.toChecksumAddress(details['purchaseToken'])
    ) {
...
async function fetchRedemption(web3, address, extAddress, balances) {
  ...
  let redemptionStarts = await redemption.methods.redemptionStarts(address).call()
...
export async function fetchRicardian(address, web3, factory, daoChain, ricardianBlock) {
  ...
  let events = await fetchEvents(contract_, web3, ricardianBlock, eventName, daoChain)
