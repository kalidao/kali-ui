import { Avatar, Card, Heading, Skeleton, Box, Spinner, Stack, Stat, Button, IconArrowRight, IconTokens, IconLightningBolt, IconCog, IconBookOpen } from '@kalidao/reality'
...
const Nav = ({ address, chainId }: DashboardElementProps) => {
  ...
  const { data: swap, isLoading: isSwapLoading, error: swapError, isError: isSwapError } = useContractRead({
  ...
  return (
    <Card padding="6" width="full">
      <Box className={navMenu}>
        {items.map(item => <NavCard title={item.title} href={item.href} icon={item.icon} active={item.active} key={item.id} />)}
      </Box>
    </Card>
  )
}
...
const NavCard = ({ title, href, icon, active }: NavCardProps) => {
    ...
    return <Link href={href} passHref><Box as="a" className={navItem} backgroundColor={active ? 'accent' : 'black'}>
            {icon}
