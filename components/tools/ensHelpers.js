...
export async function validateEns(ens, web3, error) {
  ...
  if (ens.slice(-4) === ".eth") {
    address = await web3.eth.ens.getAddress(ens).catch(() => {
      ...
      (error) ? error.toast(ens + " is not a valid Ethereum address.") : console.log(ens + " is not a valid Ethereum address.");
    return;
  } else {
    address = ens;
  }
...
export async function validateEnsList(list, web3, error) {
  ...
  for (let i = 0; i < list.length; i++) {
    if (list[i].slice(-4) === ".eth") {
      ...
      (error) ? error.toast(list[i] + " is not a valid Ethereum address.") : console.log(list[i] + " is not a valid Ethereum address.");
      return;
    } else {
      list_.push(list[i]);
    }
