...
export default function SendShares() {
  ...
  const submitProposal = async () => {
    ...
    try {
      ...
      let amounts_ = []
      for (let i = 0; i < selection.length; i++) {
        const amount_ = await instance.methods.balanceOf(selection[i].value).call()
      ...
      console.log('Shares Array', proposalType_, description_, accounts_, amounts_, payloads_)
      try {
  ...
  return (
    <VStack align="flex-start" w="70%">
      <Text fontSize="14px">Select member(s) to remove. Address with ENS will update when available.</Text>
