...
import { toDecimals, fromDecimals, getTokenName } from '../../utils/formatters'
...
export default function BuyCrowdsale() {
  ...
  const personalLimit = dao['extensions']['crowdsale']['details']['personalLimit']
  ...
  const getPurchaseTokenBalance = async () => {
    try {
      const instance_ = new web3.eth.Contract(tokenAbi, purchaseToken)
      let result = await instance_.methods.balanceOf(account).call()
  ...
  const checkPurchaseTokenAllowance = async () => {
    ...
    try {
      let result = await instance.methods.allowance(account, crowdsaleAddress).call()
      result = web3.utils.fromWei(result, 'ether')
  ...
  const submitProposal = async (event) => {
    ...
    try {
      if (purchaseToken.toLowerCase() != ether) {
        try {
          let result = await instance.methods.callExtension(address, purchaseAmount_).send({ from: account })
  ...
  const handleChange = (value) => {
    ...
    if (purchaseLimit_ > parseFloat(purchaseLimit)) {
      setError(
        `Purchase amount of ${purchaseAmount_} DAO Tokens exceeds amount available for sale, ${fromDecimals(
          purchaseLimit,
          18,
        )}.`,
      )
    ...
    if (toDecimals(purchaseAmount_, 18) > personalRemainder) {
      setError(
        `Purchase amount of ${purchaseAmount_} DAO Tokens exceeds personal limit, ${fromDecimals(personalLimit, 18)}.`,
      )
    }
  }
    ...
    <>
      <form onSubmit={submitProposal}>
        <Stack w="100%" spacing="40px">
          <Text>
            <i>{dao.name.substring(0, 1).toUpperCase() + dao.name.substring(1)}</i> is currently running a sale of its
            token with the following details:
          </Text>
          <VStack align="flex-start">
            ...
            <CrowdsaleDetail
              ...
              input={`~${purchaseMultiplierRatio.toFixed(4)} ${
                purchaseToken != ether ? purchaseTokenSymbol : 'Ether'
              } (${purchaseMultiplier} ${dao.token['symbol'].toUpperCase()} / ${
                purchaseToken != ether ? purchaseTokenSymbol : 'Ether'
              })`}
            />
            <CrowdsaleDetail
            ...
            />
            <CrowdsaleDetail name={'Eligibility: '} input={purchaseList} />
            <CrowdsaleDetail name={'Sale Progress: '} input={`ending in ${timeLeft}`} />
            <br />
            <Progress
              w="100%"
              value={(fromDecimals(purchasedAmount, 18) / fromDecimals(purchaseLimit, 18)) * 100}
              colorScheme={'green'}
              size={'sm'}
            />
            <VStack align={'flex-end'} w="100%">
              <Text>
                {fromDecimals(purchasedAmount, 18)} / {fromDecimals(purchaseLimit, 18)} (
                {((fromDecimals(purchasedAmount, 18) / fromDecimals(purchaseLimit, 18)) * 100).toFixed(2)}%)
              </Text>
            </VStack>
          </VStack>
          <>
            {eligibleBuyer ? (
              ...
              <Center>
                <Text>
                  <b>You are not eligible to participate in this crowdsale.</b>
                </Text>
              </Center>
            )}
          </>
          <Crowdsales
            sales={dao['extensions']['crowdsale']['details']['purchase']}
            symbol={dao.token['symbol'].toUpperCase()}
          />
        </Stack>
      </form>
    </>
