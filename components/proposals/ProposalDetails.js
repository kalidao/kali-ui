      ...
      {p['amounts'].map((item, index) => (
        <>
          <Box key={`item-${index}`} background="#eeeeee" p={5} mb={5}>
            ...
            {details['amounts'] == null ? null : p['amounts'][index] == 0 ? null : (
            ...
            {details['accounts'] == null ? null : (
              <div key={`accounts-${index}`}>
                <ProposalLabel>{p['payloads'][index] == '0x' ? 'Recipient' : details['accounts']}</ProposalLabel>
                <Text>
                  {p['payloads'][index] == '0x' ? (
                    p['accounts'][index]
                  ) : (
            ...
            {details['payloads'] == null ? null : p['payloads'][index] != '0x' ? ( // don't display dummy data
